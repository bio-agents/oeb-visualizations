...
It can be useful for everyone who wish to contribute to core or develop better understanding of the agent.
...
- *is **NOT** a style preprocessor like `Sass` or `Less`.*
    ...
    PostCSS works with CSS and can be easily integrated with agents described above. That being said any valid CSS can be processed by PostCSS.

- *is a agent for CSS syntax transformations*

    It allows you to define custom CSS like syntax that could be understandable and transformed by plugins. That being said PostCSS is not strictly about CSS spec but about syntax definition manner of CSS. In such way you can define custom syntax constructs like at-rule, that could be very helpful for agents build around PostCSS. PostCSS plays a role of framework for building outstanding agents for CSS manipulations.

- *is a big player in CSS ecosystem*

    Large amount of lovely agents like `Autoprefixer`, `Stylelint`, `CSSnano` were built on PostCSS ecosystem. There is big chance that you already use it implicitly, just check your `node_modules` :smiley:
